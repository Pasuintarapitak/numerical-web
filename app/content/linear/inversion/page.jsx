...
export default function GaussianElimination() {
    ...
    const [matrixIn, setMatrixIn] = useState([]); 
    ...
    const matrixInversion = () => {
        ...
        // let I = [];
        let jordanSteps = [];
        //initial Inversion matrix
        let I = Array.from({ length: size }, () => Array(size).fill(0));
        ...
        for (let i = 0; i < size; i++) {
            ...
            for (let j = 0; j < size; j++) {
                if (i !== j) {
                    ...
                    for (let k = 0; k < size; k++) {
                        A[j][k] -= ratio * A[i][k]; // Update the row
                        I[j][k] -= ratio * I[i][k];
                    }
            ...
            jordanSteps.push({
                ...
                matrix: A.map(row => row.map(val => val.toFixed(2)).join(' & ')).join(' \\\\ '),
                matrixI: I.map(row => row.map(val => val.toFixed(2)).join(' & ')).join(' \\\\ '),
    ...
    return (
        <div>
            <div className='mt-8 text-center text-4xl header'>Matrix Inversion Method</div>
                ...
                {isCalculated && (
                    <div className="flex flex-col items-center mt-8">
                        ...
                        {steps.map((step, index) => {
                            // สร้างสตริงเมทริกซ์ที่ขีดขวางระหว่าง A และ I
                            const augmentedMatrix = step.matrix.split('\\\\').map((row, rowIndex) => {
                                return `${row} & | & ${step.matrixI.split('\\\\')[rowIndex]}`; // เพิ่ม ' | ' เป็นเส้นขีดขวาง
                            }).join('\\\\');

                            return (
                                <div key={index} className="mb-6">
                                    <BlockMath math={`\\begin{bmatrix} ${augmentedMatrix} \\end{bmatrix}`} />
                                </div>
                            );
                        })}
